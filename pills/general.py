...
from utils.theme import (
    ...
    PRIMARY, PRIMARY_40, PALETTE_30, PALETTE_SUCURSALES,
    PALETTE_DIRECCIONES, TEXT_MUTED, TEXT_MAIN,
)
...
def render(df: pd.DataFrame):
    ...
    with col_dir:
        ...
        display = dir_df.rename(columns={
            "subdireccion":"Dirección","creditos":"Créditos","cartera":"Cartera",
            "prom_prestamo":"Prom. Préstamo","prom_semanal":"Prom. Semanal",
    ...
    # ── Créditos por mes — un solo color (son todos créditos, misma categoría)
    ...
    fig_mes.add_bar(
        ...
        marker_color=PRIMARY_40, marker_line_color=PRIMARY, marker_line_width=1.5,
        hovertemplate="<b>%{x}</b><br>Créditos: %{y:,}<extra></extra>",
    )
    apply_theme(fig_mes, "Número de Créditos por Mes")
    fig_mes.update_layout(height=300, xaxis_title="Mes", yaxis_title="Número de Créditos")
    st.plotly_chart(fig_mes, use_container_width=True)

    # ── Créditos por mes × sucursal — paleta de sucursales ───────────────────
    ...
    suc_filter = st.multiselect(
        "Seleccionar sucursales", options=top15, default=top15[:8], key="suc_mes_filter",
    )
    if suc_filter:
        pivot = (
            df[df["sucursal"].isin(suc_filter)]
    ...
    st.markdown(section_title("Créditos y Monto Otorgado"), unsafe_allow_html=True)
    granularity = st.radio("Granularidad", ["Mensual","Semanal"], horizontal=True, key="gran_radio")

    if granularity == "Mensual":
        grp = (df.groupby(["mes_orden","mes_nombre"], as_index=False)
               .agg(creditos=("prestamo","count"), monto=("monto_prestamo","sum"))
               .sort_values("mes_orden"))
        x_col, x_label = "mes_nombre", "Mes"
    else:
        grp = (df.groupby(["semana_orden","semana_str"], as_index=False)
               .agg(creditos=("prestamo","count"), monto=("monto_prestamo","sum"))
    ...
    with col_l:
        ...
        fig_c.update_layout(height=300, xaxis_tickangle=-30, showlegend=False,
    ...
    with col_r:
        ...
        fig_m.add_bar(x=grp[x_col], y=grp["monto"],
                      marker_color="rgba(102,204,136,0.35)", marker_line_color=COLOR2, marker_line_width=1.2,
                      hovertemplate=f"<b>%{{x}}</b><br>Monto: $%{{y:,.0f}}<extra></extra>")
        fig_m.add_scatter(x=grp[x_col], y=grp["monto"],
                          mode="lines+markers", line=dict(color=COLOR2, width=2),
                          marker=dict(size=5), hoverinfo="skip")
        apply_theme(fig_m, f"Monto Otorgado ({granularity})")
        fig_m.update_layout(height=300, xaxis_tickangle=-30, showlegend=False,
                            xaxis_title=x_label, yaxis_title="Monto Otorgado ($)")
        st.plotly_chart(fig_m, use_container_width=True)

    # ── Top 10 sucursales — paleta de sucursales (una por barra = una sucursal)
    ...
    top10 = (
        ...
        .nlargest(10,"cartera").sort_values("cartera")
    )
    fig_top = px.bar(
        top10, x="cartera", y="sucursal", orientation="h",
        color="sucursal",
        color_discrete_sequence=PALETTE_SUCURSALES,
    ...
    fig_top.update_layout(height=380, showlegend=False,
                          xaxis_title="Cartera Total ($)", yaxis_title="")
