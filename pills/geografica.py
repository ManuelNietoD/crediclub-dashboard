...
from utils.theme import (
    ...
    PRIMARY, PRIMARY_40, TEXT_MUTED, TEXT_MAIN, BAR_COLORS
)

# Approximate lat/lon centroids per Subdireccion for bubble map
REGION_COORDS = {
...
def render(df: pd.DataFrame):
    ...
    # ── Heatmap: Monto Préstamo × Región × Mes ───────────────────────────────
    st.markdown(section_title("Heatmap — Monto Préstamo por Región × Mes"), unsafe_allow_html=True)
    ...
    # pivot wide
    mes_labels = pivot.sort_values("mes_orden")[["mes_orden", "mes_nombre"]].drop_duplicates().set_index("mes_orden")["mes_nombre"]
    pivot_wide = pivot.pivot_table(index="region", columns="mes_nombre", values="monto", aggfunc="sum", fill_value=0)
    ...
    fig_heat = px.imshow(
        ...
        aspect="auto",
        labels={"color": "Monto Préstamo"},
        text_auto=".2s",
    )
    fig_heat.update_traces(
        textfont=dict(color="white", size=10),
        hovertemplate="<b>%{y}</b><br>%{x}<br>Monto: $%{z:,.0f}<extra></extra>",
    )
    ...
    fig_heat.update_layout(
        ...
        xaxis=dict(tickangle=-30, tickfont=dict(color=TEXT_MUTED, size=11)),
        yaxis=dict(tickfont=dict(color=TEXT_MUTED, size=11)),
    )
    st.plotly_chart(fig_heat, use_container_width=True)

    # ── Mapa burbuja de México ──────────────────────────────────────────────
    st.markdown(section_title("Mapa Geográfico — Cartera por Dirección"), unsafe_allow_html=True)
    ...
    # ── Monto promedio pago semanal por dirección ────────────────────────────
    st.markdown(section_title("Monto Promedio Pago Semanal por Dirección"), unsafe_allow_html=True)
    ...
    fig_semanal = px.bar(
        ...
        labels={"prom_semanal": "Pago Semanal Promedio", "subdireccion": ""},
        text=pago_dir["prom_semanal"].apply(fmt_currency),
    )
    fig_semanal.update_traces(textposition="outside", textfont=dict(color=TEXT_MUTED, size=11))
    apply_theme(fig_semanal, "Promedio de Pago Semanal por Dirección")
    fig_semanal.update_layout(height=340, coloraxis_showscale=False)
    st.plotly_chart(fig_semanal, use_container_width=True)

    # ── Treemap: Dirección → Región → Sucursal ────────────────────────────────
    st.markdown(section_title("Treemap — Cartera: Dirección → Región → Sucursal"), unsafe_allow_html=True)
    ...
    fig_tree.update_layout(height=500, coloraxis_showscale=False,
