...
from utils.theme import (
    ...
    PRIMARY, PRIMARY_40, BAR_COLORS, TEXT_MUTED,
)
...
def render(df: pd.DataFrame, emp_raw: pd.DataFrame):
    ...
    with st.sidebar:
        st.markdown(
            ...
            f'letter-spacing:0.1em;margin:16px 0 8px 0">Filtros</div>',
    ...
    for col, label, val, sub in [
        (c1, "Sucursales",       f"{total_suc}",         "en filtro seleccionado"),
        (c2, "Empleados Activos",f"{emp_activos:,}",     f"de {total_emp:,} totales"),
        (c3, "Créditos",         f"{total_cred:,}",      "en filtro seleccionado"),
    ...
    top_n = st.slider("Número de sucursales a mostrar", 5, 30, 12, key="top_n_suc")
    ...
    col_sort = st.selectbox(
        "Ordenar por",
        ["Cartera","Créditos","% Mora","Monto Colocado"],
        key="rank_sort",
    )
    sort_map = {
    ...
    fig_sc = px.scatter(
        ranking,
        x="Cartera", y="% Mora",
        size="Créditos", color="Dirección",
        color_discrete_sequence=BAR_COLORS * 2,
        hover_name="Sucursal",
        labels={"Cartera":"Cartera Total ($)","% Mora":"% de Mora","Créditos":"Créditos"},
        size_max=40,
    )
